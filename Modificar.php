  ...
  if(!isset($_POST['btnModificar'])){
        ...
        if (file_exists($ruta)) {
          ...
          if($bomberoDAO->modificarBombero($bombero)==1 && $infoDAO->modificarInfo($info)==1){
            echo '<script languaje="javascript">alertaModificar(1)</script> ';
          }else{
            echo '<script languaje="javascript">alertaModificar(0);</script> ';
          }
  }
 ?>

<div class="contBotonesM">
  <button type="button" class="btnForm"  onclick="window.location.href='Listado.php'" name="button">Volver</button>
</div>
</div>
